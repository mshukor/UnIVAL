...
from data.audio_utils import AUDIO_CFG
...
class OFAConfig(FairseqDataclass):
    ...
    ## data
    read_from_img_path: bool = field(
        default=False,
        metadata={"help": "read from image paths, don't convert images to str"},
    )

    image_dir: str = field(
        default="/lus/scratch/NAT/gda2204/SHARED/data",
        metadata={"help": "path to datasets']"},
    )
    
    no_image_transform: bool = field(
        default=False,
        metadata={"help": "read from image paths, don't convert images to str"},
    )
    patch_frame_size: int = field(
        default=224, metadata={"help": "patch_frame_size"}
    )

    num_frames: int = field(
        default=4, metadata={"help": "patch_frame_size"}
    )

    # audio 
    max_audio_len: int = field(
        default=480000, metadata={"help": "patchmax_audio_len_frame_size"}
    )
    audio_cfg: Optional[str] = field(
        default=None, metadata={"help": '{"beam":5,"unnormalized":true,"temperature":1.0}'}
    )

    sample_type: str = field(
        default="rand",
        metadata={"help": "sample_type']"},
    )

    sample_rate: int = field(
        default=48000, metadata={"help": "sample rate"}
    )

    audio_sample_rate: bool = field(
        default=False,
    ...
    ast: bool = field(
        default=False,
        metadata={"help": "Use AST audio encoder"},
    )

    mel_bins: int = field(
        default=64, metadata={"help": "mel_bins"}
    )

    use_dataaug: bool = field(
        default=False,
        metadata={"help": "Use data augmentation"},
    )


@register_task("unival", dataclass=OFAConfig)
class OFATask(FairseqTask):
    ...
    def build_generator(
        ...
        stop_on_max_len = getattr(args, "stop_on_max_len", False)
