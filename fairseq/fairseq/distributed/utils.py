...
def distributed_init(cfg: FairseqConfig):
    ...
    if not cfg.common.tpu:
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            ...
            logger.info(
                "distributed init (rank {}): {}, world_size {}, backend: {}".format(
            ...
            torch.cuda.set_device(cfg.distributed_training.device_id) # to fix NCCL error!!
...
def distributed_main(i, main, cfg: FairseqConfig, kwargs):
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        i = int(os.environ['LOCAL_RANK'])
    elif 'SLURM_PROCID' in os.environ:
        # args.rank = int(os.environ['SLURM_PROCID'])
        i = cfg.distributed_training.distributed_rank % torch.cuda.device_count()
...
def broadcast_tensors(
    ...
    for i, meta in enumerate(metadata):
        if is_src_rank:
            tensor = tensors[i]
            try:
                broadcast(tensors[i].to(dist_device), src=src_rank, group=group) #.contiguous()
            except: # error for audio experiments
                broadcast(tensors[i].to(dist_device).contiguous().cuda(), src=src_rank, group=group)
                print("non contiguous tensor, casted to contiguous:", i, tensors[i].shape, dist_device)
        else:
            tensor = torch.zeros(
