...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss, save_on_cuda=False):
    ...
    if len(checkpoints) > 0:
        trainer.save_checkpoint(checkpoints[0], extra_state, save_on_cuda=save_on_cuda)
...
def load_checkpoint(cfg: CheckpointConfig, trainer, strict=True, load_on_cuda=False, **passthrough_args):
    """
    ...
    *passthrough_args* will be passed through to
    ...
    if extra_state is not None and not reset_dataloader:
        ...
        if hasattr(epoch_itr.dataset, 'pure_text_dataset') and epoch_itr.dataset.pure_text_dataset is not None:
        ...
        if hasattr(epoch_itr.dataset, 'pure_image_dataset') and epoch_itr.dataset.pure_image_dataset is not None:
        ...
        if hasattr(epoch_itr.dataset, 'detection_dataset') and epoch_itr.dataset.detection_dataset is not None:
...
def load_checkpoint_to_cpu(path, arg_overrides=None, load_on_all_ranks=False, strict=True, load_on_cuda=False):
    ...
    with open(local_path, "rb") as f:
        state = torch.load(f, map_location=map_device)
...
def load_model_ensemble_and_task(
    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            if state is None:
                state = load_checkpoint_to_cpu(filename, arg_overrides, strict=strict)
            ...
            if "fsdp_metadata" in state and num_shards > 1:
                ...
                #### 
                delete_keys = [] # for audio encoder 
                for n,p in state["model"].items():
                    if "logmel_extractor" in n:
                        delete_keys.append(n)
            ...
            if "extra_state" in state:
                print(state['extra_state'].get("best", None), state['extra_state'].get("train_iterator", None))
...
def _upgrade_state_dict(state):
    ...
    if "optimizer_history" not in state:
        state["optimizer_history"] = [
            {"criterion_name": "CrossEntropyCriterion", "best_loss": state.get("best_loss", 0)}
        ]
